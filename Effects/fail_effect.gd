...
var amount := 0
var _last_message_index := 1 #start of random message things
var messages := [
	...
	"Your attack lands somewhere in another dimension.",
	"You miss. Spectacularly. A butterfly is unharmed, thankfully.",
...
func execute(targets: Array[Node]) -> void:
	for target in targets:
		...
		if target is Enemy or target is CombatPlayer:
				var message := get_new_random_message() #part of random message
